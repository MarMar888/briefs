...
export default async function Page({
  ...
  const filters: LeadFilters = {
    minScore: numberParam(first(queryParams.minScore)),
    maxScore: numberParam(first(queryParams.maxScore)),
    ecomOnly: (first(queryParams.ecomOnly) as LeadFilters["ecomOnly"]) || "all",
    reviewed: (first(queryParams.reviewed) as LeadFilters["reviewed"]) || "approved",
    audit: (first(queryParams.audit) as LeadFilters["audit"]) || "qualified",
    industry
  };

  const [stats, leads] = await Promise.all([getLeadStats(industry), getMatchedLeads(filters)]);
