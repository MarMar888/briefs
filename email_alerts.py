...
def _format_text(matches: list[dict]) -> str:
    lines = [f"New OSI domain leads: {len(matches)}", ""]
...
def _format_html(matches: list[dict]) -> str:
    rows = []
    for match in matches:
...
def send_match_alerts(matches: list[dict]) -> bool:
    """Send one digest email for newly matched domains."""
    if not matches:
        return True
    ...
    sender = os.environ.get("ALERT_FROM") or "OSI Lead Monitor <onboarding@resend.dev>"
        ...
        resend.Emails.send(
            {
                "from": sender,
                "to": [alert_email],
                "subject": f"{len(matches)} new OSI domain lead(s)",
                "html": _format_html(matches),
                "text": _format_text(matches),
            }
        )
